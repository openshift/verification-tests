Feature: pod related features
  ...
  Scenario: Existing pods will not be affected when node is unschedulable
    ...
    When I run the :create client command with:
      | f | <%= ENV['BUSHSLICER_HOME'] %>/testdata/pods/pod-pull-by-tag.yaml |
    Then the step should succeed
  ...
  Scenario: pod node label selector must be consistent with its project node label selector
    ...
    When I run the :create admin command with:
      | f | <%= ENV['BUSHSLICER_HOME'] %>/testdata/pods/pod-with-nodeselector.yaml |
      | n | <%= cb.proj_name %>                                                                                |
  ...
  Scenario: Pod will not be copied to nodes which does not match it's node selector
    ...
    When I run the :create client command with:
      | f | <%= ENV['BUSHSLICER_HOME'] %>/testdata/daemon/daemonset-nodeselector.yaml |
    Then the step should succeed
  ...
  Scenario: Secret is valid after node reboot
    ...
    Given I store the schedulable nodes in the :nodes clipboard
    When I run the :create client command with:
      | f | <%= ENV['BUSHSLICER_HOME'] %>/testdata/secrets/tc483170/secret-nginx-2.yaml |
    Then the step should succeed
    When I run the :get client command with:
      | resource      | secret         |
  ...
  Scenario: Pod and container level selinuxoptions should both work
    ...
    Given SCC "privileged" is added to the "default" user
    When I run the :create client command with:
      | f | <%= ENV['BUSHSLICER_HOME'] %>/testdata/pods/securityContext/pod-selinux.yaml |
    Then the step should succeed
    ...
    When I run the :create client command with:
      | f | <%= ENV['BUSHSLICER_HOME'] %>/testdata/pods/securityContext/container-selinux.yaml |
    Then the step should succeed
    ...
    And the output should contain:
      | unconfined_u:unconfined_r:svirt_lxc_net_t:s0:c25,c968 |
    When I run the :create client command with:
      | f | <%= ENV['BUSHSLICER_HOME'] %>/testdata/pods/securityContext/both-level.yaml |
    Then the step should succeed
