Feature: system metric related tests
  ...
  Scenario: Access the external Hawkular Metrics API interface as cluster-admin
    ...
    Given metrics service is installed in the system using:
      | inventory       | <%= ENV['BUSHSLICER_HOME'] %>/testdata/logging_metrics/OCP-11821/inventory              |
      | deployer_config | <%= ENV['BUSHSLICER_HOME'] %>/testdata/logging_metrics/OCP-11821/deployer_ocp11821.yaml |
