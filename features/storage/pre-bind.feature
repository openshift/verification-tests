Feature: Testing for pv and pvc pre-bind feature
  ...
  Scenario: Prebound pv is availabe due to requested pvc status is bound
    ...
    Given admin creates a PV from "<%= ENV['BUSHSLICER_HOME'] %>/testdata/storage/nfs/nfs.json" where:
      | ["metadata"]["name"]         | pv1-<%= project.name %> |
      | ["spec"]["storageClassName"] | sc-<%= project.name %>  |
    Then I create a dynamic pvc from "<%= ENV['BUSHSLICER_HOME'] %>/testdata/storage/nfs/claim-rwo.json" replacing paths:
      | ["metadata"]["name"]         | mypvc                  |
      | ["spec"]["storageClassName"] | sc-<%= project.name %> |
    And the "mypvc" PVC becomes bound to the "pv1-<%= project.name %>" PV
    Then admin creates a PV from "<%= ENV['BUSHSLICER_HOME'] %>/testdata/storage/nfs/preboundpv-rwo.yaml" where:
  ...
  Scenario: Prebound pv is availabe due to mismatched accessmode with requested pvc
    Given I have a project
    Given admin creates a PV from "<%= ENV['BUSHSLICER_HOME'] %>/testdata/storage/nfs/preboundpv-rwo.yaml" where:
  ...
  Scenario: Prebound pvc is pending due to requested pv status is bound
    ...
    Then I create a dynamic pvc from "<%= ENV['BUSHSLICER_HOME'] %>/testdata/storage/nfs/preboundpvc-rwo.yaml" replacing paths:
  ...
  Scenario: PVC is bond to PV successfully when pvc is created first
    ...
    Then I create a dynamic pvc from "<%= ENV['BUSHSLICER_HOME'] %>/testdata/storage/nfs/claim-rwo.json" replacing paths:
      | ["metadata"]["name"]         | mypvc                  |
      | ["spec"]["storageClassName"] | sc-<%= project.name %> |
    Then admin creates a PV from "<%= ENV['BUSHSLICER_HOME'] %>/testdata/storage/nfs/nfs-recycle-rwo.json" where:
      | ["metadata"]["name"]         | pv-<%= project.name %> |
      | ["spec"]["storageClassName"] | sc-<%= project.name %> |
