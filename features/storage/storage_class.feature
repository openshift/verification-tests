Feature: storageClass related feature
  ...
  Scenario Outline: PVC modification after creating storage class
    ...
    When I create a manual pvc from "<%= ENV['BUSHSLICER_HOME'] %>/testdata/storage/misc/pvc-without-annotations.json" replacing paths:
      | ["metadata"]["name"] | mypvc |
  ...
  Scenario Outline: No dynamic provision when no default storage class
    ...
    When I run oc create over "<%= ENV['BUSHSLICER_HOME'] %>/testdata/storage/misc/pvc-without-annotations.json" replacing paths:
      | ["metadata"]["name"] | mypvc |
    Then the step should succeed
    And the "mypvc" PVC becomes :pending
    Given 30 seconds have passed
  ...
  Scenario Outline: storage class provisioner
    ...
    When I run oc create over "<%= ENV['BUSHSLICER_HOME'] %>/testdata/storage/misc/pod.yaml" replacing paths:
      | ["metadata"]["name"]                                         | mypod     |
  ...
  Scenario Outline: PVC with storage class will provision pv with io1 type and 100/20000 iops ebs volume
    Given I have a project
    When admin creates a StorageClass from "<%= ENV['BUSHSLICER_HOME'] %>/testdata/storage/ebs/dynamic-provisioning/storageclass-io1.yaml" where:
      | ["metadata"]["name"] | sc-<%= project.name %> |
    Then the step should succeed
  ...
  Scenario Outline: PVC with storage class will not provision pv with st1/sc1 type ebs volume if request size is wrong
    Given I have a project
    When admin creates a StorageClass from "<%= ENV['BUSHSLICER_HOME'] %>/testdata/storage/ebs/dynamic-provisioning/storageclass.yaml" where:
      | ["metadata"]["name"]   | sc-<%= project.name %> |
  ...
  Scenario: AWS ebs volume is dynamic provisioned with default storageclass
    Given I have a project
    When I run oc create over "<%= ENV['BUSHSLICER_HOME'] %>/testdata/storage/ebs/pvc-retain.json" replacing paths:
      | ["metadata"]["name"]                         | mypvc |
      | ["spec"]["resources"]["requests"]["storage"] | 1Gi                     |
    Then the step should succeed
