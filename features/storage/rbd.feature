Feature: Storage of Ceph plugin testing
  ...
  Scenario: Dynamically provision Ceph RBD volumes
    ...
    And I run oc create over "<%= BushSlicer::HOME %>/testdata/storage/rbd/dynamic-provisioning/user_secret.yaml" replacing paths:
      | ["data"]["key"] | <%= cb.secret_key %> |
    When I run oc create over "<%= BushSlicer::HOME %>/testdata/storage/rbd/dynamic-provisioning/pod.json" replacing paths:
