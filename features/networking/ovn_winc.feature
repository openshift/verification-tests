Feature: OVNKubernetes Windows Container related networking scenarios
  ...
  Scenario: Ensure Pods and Service communication across window and linux nodes
    ...
    And evaluation of `service.ip(user: user)` is stored in the :linux_service_ip clipboard
    ...
    And evaluation of `service.ip(user: user)` is stored in the :windows_service_ip clipboard
