Feature: test new api methods
  ...
  Scenario: clusterversion apis
    ...
    And evaluation of `cluster_version('version').history_matching(query_hash: {'version' => cluster_version('version').version})` is stored in the :history_match clipboard
