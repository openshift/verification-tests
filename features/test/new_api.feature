Feature: test new api methods
  ...
  Scenario: clusterversion apis
    ...
    And evaluation of `cluster_version('version').wait_for_upgrade_completion(version: ENV['UPGRADE_TARGET_VERSION'], upgrade_timeout: 10)` is stored in the :upgrade_status clipboard
