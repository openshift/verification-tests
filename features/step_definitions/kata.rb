...
Given /^I wait until sandboxed operator is ready$/ do
  ...
  operator_status[:success] = wait_for(timeout, stats: {}) do
    ...
    if cb.master_version == "4.8"
      @result = admin.cli_exec(:get, resource: "operators", o: "jsonpath=''{..status.components.refs[7].conditions[0].type}'", n:kata_ns)[:stdout].to_s.include? expected_status
      @result = admin.cli_exec(:get, resource: "operators", o: "jsonpath=''{..status.components.refs[8].conditions[0].type}'", n:kata_ns)[:stdout].to_s.include? expected_state
    else
