...
Given /^current generation number of#{OPT_QUOTED} deployment under #{QUOTED} is stored into#{OPT_SYM} clipboard$/ do |name,namespace,cb_name|
  ensure_admin_tagged
  ensure_destructive_tagged
  cb_name ||= :generation_number
  @result = admin.cli_exec(:get, resource: 'deployment', resource_name: name, n: namespace, o: 'yaml')
