...
Given /^docker config for default image registry is saved$/ do
  ...
  cb.dockercfg_file="/tmp/#{cb.dockercfg_name}"
  ...
  File.open("#{cb.dockercfg_file}", 'wb') { |f| 
    f.write(
       {   
         "auths" => generated_cfg
       }.to_json
