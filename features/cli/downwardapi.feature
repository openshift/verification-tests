Feature: Downward API
  ...
  Scenario: Pods can get IPs via downward API under race condition
    ...
    When I run the :create client command with:
      | filename  | <%= ENV['BUSHSLICER_HOME'] %>/testdata/downwardapi/tc509097/pod-downwardapi-env.yaml |
  ...
  Scenario: downward api pod name and pod namespace as env variables
    Given I have a project
    When I run the :create client command with:
      | f | <%= ENV['BUSHSLICER_HOME'] %>/testdata/downwardapi/tc483203/downward-example.yaml |
    Then the step should succeed
  ...
  Scenario: Container consume infomation from the downward API using a volume plugin
    Given I have a project
    When I run the :create client command with:
      | f | <%= ENV['BUSHSLICER_HOME'] %>/testdata/downwardapi/pod-dapi-volume.yaml |
    Then the step should succeed
  ...
  Scenario: Using resources downward API via volume plugin should be compatible with metadata downward API
    Given I have a project
    When I run the :create client command with:
      | f | <%= ENV['BUSHSLICER_HOME'] %>/testdata/downwardapi/dapi-resources-metadata-volume-pod.yaml |
    Then the step should succeed
    ...
    When I run the :create client command with:
      | f | <%= ENV['BUSHSLICER_HOME'] %>/testdata/downwardapi/dapi-resources-metadata-volume-pod-without-requests.yaml |
    Then the step should succeed
    ...
    When I run the :create client command with:
      | f | <%= ENV['BUSHSLICER_HOME'] %>/testdata/downwardapi/dapi-resources-metadata-volume-pod-without-limits.yaml |
    Then the step should succeed
  ...
  Scenario: Could expose resouces limits and requests via volume plugin from Downward APIs with magics keys
    Given I have a project
    When I run the :create client command with:
      | f | <%= ENV['BUSHSLICER_HOME'] %>/testdata/downwardapi/dapi-resources-volume-magic-keys-pod.yaml |
    Then the step should succeed
    ...
    When I run the :create client command with:
      | f | <%= ENV['BUSHSLICER_HOME'] %>/testdata/downwardapi/dapi-resources-volume-magic-keys-pod-without-requests.yaml |
    Then the step should succeed
    ...
    When I run the :create client command with:
      | f | <%= ENV['BUSHSLICER_HOME'] %>/testdata/downwardapi/dapi-resources-volume-magic-keys-pod-without-limits.yaml |
    Then the step should succeed
  ...
  Scenario: Using resources downward API via ENV should be compatible with metadata downward API
    Given I have a project
    When I run the :create client command with:
      | f | <%= ENV['BUSHSLICER_HOME'] %>/testdata/downwardapi/dapi-resources-metadata-env-pod.yaml |
    Then the step should succeed
    ...
    When I run the :create client command with:
      | f | <%= ENV['BUSHSLICER_HOME'] %>/testdata/downwardapi/dapi-resources-metadata-env-pod-without-requests.yaml |
    Then the step should succeed
    ...
    When I run the :create client command with:
      | f | <%= ENV['BUSHSLICER_HOME'] %>/testdata/downwardapi/dapi-resources-metadata-env-pod-without-limits.yaml |
    Then the step should succeed
