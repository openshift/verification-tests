...
module BushSlicer
  class EnvLauncherCli
    ...
    def terminate(vminfo)
      dns_component = vminfo.delete(:dns_component)
      vminfo.each do |target, spec|
        ...
        when /^template_/
          ...
          #dns_component only used in 3.x install
          if (not dns_component.nil?) and (not dns_component.empty?)
            logger.info "Deleting DNS records in AWS Route53..."
            aws_iaas = iaas_by_service("AWS-CI")
            aws_iaas.delete_resource_records_re(/.+[.]#{dns_component}/)
          end
        end
      end
    end
