...
module BushSlicer
  class EnvLauncherCli
    ...
    def gen_ose3_timed_random_component
      return Time.now.strftime("%m%dos3") << "-" << rand_str(2, :dns)
    end
    ...
    def terminate(vminfo)
      ...
      vminfo.each do |target, spec|
        cluster_names = spec.map {|s| s[:name]}
      ...
      # try to remove the corresponding route53 entries
      # 1. try to find the dns_record from the viminfo.yaml by querying for the
      # route53
      cluster_prefix = cluster_names[0].split('master').first
      dns_record_prefix = cluster_prefix.gsub("_","-")
      begin
        logger.info "Deleting DNS records in AWS Route 53..."
        aws_iaas = iaas_by_service("AWS-CI")
        records = iaas_by_service('AWS-CI').list_resource_record_sets
        name_records = records.select { |r| r[:name] =~ /^#{dns_record_prefix}/ }
        if name_records.count > 0
          dns_record = name_records[0][:name].split('.qe.devcluster.openshift.com.').last.split('.').last
          aws_iaas.delete_resource_records_re(Regexp.new(dns_record))
          logger.info "DNS records in AWS Route 53 were deleted successfully."
        else
          logger.info("No DNS records found matching #{dns_record_prefix}")
        end
      rescue => e
        logger.error "Error during deleting AWS Route 53 DNS records: #{exception_to_string(e)}"
      end
    end
